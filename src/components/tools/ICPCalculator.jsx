...
const ICPCalculator = () => {
    ...
    return (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden flex flex-col h-full border-t-4 border-t-purple-600">
            <div className="p-6 border-b border-slate-100 bg-white z-20 flex justify-between items-start">
                <div>
                    <div className="flex items-center gap-2 text-purple-600 font-mono text-xs uppercase tracking-widest mb-1">
                        <Target className="w-4 h-4" /> Qualification Radar
                    </div>
                    <h2 className="text-2xl font-bold text-slate-900">ICP Signal Scorer</h2>
                </div>
            ...
            <div className="flex-grow p-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
                {/* CHART */}
                <div className="bg-white rounded-2xl border border-slate-200 p-4 relative flex flex-col items-center justify-center min-h-[300px]">
                    <ResponsiveContainer width="100%" height="100%">
                        <RadarChart cx="50%" cy="50%" outerRadius="80%" data={data}>
                            ...
                            <Radar
                                ...
                                strokeWidth={2}
                            />
                            <Tooltip contentStyle={{ backgroundColor: '#fff', borderColor: '#e2e8f0', borderRadius: '12px', color: '#1e293b' }} />
                        </RadarChart>
                    </ResponsiveContainer>
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none">
                        <div className={`text-4xl font-black ${signal.text}`}>{totalScore}</div>
                        <div className="text-[10px] text-slate-400 uppercase font-black tracking-widest">Aggregate</div>
                    </div>
                </div>

                {/* INPUTS */}
                <div className="space-y-6 flex flex-col justify-center">
                    <div className="bg-purple-50 border border-purple-100 p-4 rounded-xl mb-2">
                        <div className="flex items-center gap-3">
                            <div className={`p-2 rounded-lg bg-white border border-slate-100 shadow-sm ${signal.text}`}>
                                <Star className="w-5 h-5 fill-current opacity-80" />
                            </div>
                            <div>
                                <div className="text-[10px] text-purple-600 uppercase font-bold tracking-widest">Quality Diagnosis</div>
                            ...
                            return (
                                <div key={item.subject}>
                                    <div className="flex justify-between items-center text-xs mb-2">
                                        <span className="text-slate-500 font-bold uppercase tracking-wider">{item.subject}</span>
                                        <span className="text-slate-700 font-mono bg-slate-50 px-2 py-0.5 rounded border border-slate-200">{scores[key]} / 20</span>
                                    </div>
                                    <input
                                        type="range"
                                        min="0"
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50 flex flex-col md:flex-row gap-4 items-center">
                <input
                    type="text"
                    placeholder="Account Name (e.g. Acme Corp)"
                    value={leadName}
                    onChange={(e) => setLeadName(e.target.value)}
                    className="flex-grow bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-purple-500 outline-none transition-all text-slate-900"
                />
                {!status ? (
                    <button onClick={handleSave} className="w-full md:w-auto px-8 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-lg shadow-purple-900/10 active:scale-95">
                        Log Signal Result
                    </button>
                ) : (
                    <div className="text-emerald-600 text-xs font-bold px-8 py-3 flex items-center gap-2">
